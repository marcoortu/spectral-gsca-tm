...
behaviour of the ILR-Spectral-GSCA estimator along three complementary axes:
...
which is exactly the generative process assumed by sgscatm:
%
\begin{align}
...
as above.
For each replicate we fit sgscatm (with $\lambda=1$ and Procrustes
...
\paragraph{Design.}
We compare sgscatm with STM~\citep{roberts2014stm} on the primary
...
For sgscatm the path coefficients are obtained directly as
...
\begin{table}[t]
\centering
\caption{Structural comparison: \texttt{sgscatm} vs.\ STM. MSE of $\hat{\mathbf{B}}_z$ after Procrustes alignment, and computation time. $K=5$, $P=3$, $N=500$, $\sigma_\varepsilon=0.3$.}
\label{tab:block3}
...
methods are competitive: at $M=1{,}000$ sgscatm achieves
...
$M=5{,}000$ respectively) compared to sgscatm ($0.077$ and
$0.080$).
This gap is expected: when ILR scores are large, the
first-order linearisation of the closure map introduces a
non-negligible bias in the sgscatm objective, whereas STM's
variational EM operates directly on the simplex.
The tradeoff is computational: sgscatm is $8.9\times$ faster at
$M=1{,}000$ and over $90\times$ faster at $M=5{,}000$, where
STM requires $\approx 20$\,s versus sgscatm's $0.2$\,s.
The speedup grows with $M$ because STM's EM complexity scales
super-linearly while the eigendecomposition in sgscatm is
...
\begin{figure}[t]
  ...
  \caption{Distribution of MSE$(\hat{\mathbf{B}}_z)$ over $20$
           replicates for sgscatm (purple) and STM (orange),
...
\begin{figure}[t]
  ...
  \caption{Mean computation time (seconds, log scale) as a function
           of corpus size $M$, for sgscatm and STM.
           The non-iterative spectral estimator of sgscatm achieves
...
\begin{enumerate}
  ...
  \item \textbf{Computational efficiency and accuracy tradeoff.}
        ...
        approximation error in the sgscatm objective.
