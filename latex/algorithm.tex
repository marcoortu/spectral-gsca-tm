...
Algorithm~\ref{(alg:ilr_spectral_gsca)} computes the globally optimal spectral
...
% ---- Algorithm 1 --------------------------------------------------
\begin{algorithm}[h]
\caption{ILR-Spectral-GSCA Spectral Fit}
...
\noindent
Algorithm~\ref{(alg:ilr_spectral_gsca)} is \emph{non-iterative}: it consists
...
Algorithm~\ref{(alg:ilr_spectral_gsca)}---which lie on the simplex and enjoy
...
\label{alg:refine}
\begin{algorithmic}[1]
\REQUIRE Spectral fit from Algorithm~\ref{(alg:ilr_spectral_gsca)}:
...
\begin{enumerate}
  \item \textbf{Spectral fit}
    (Algorithm~\ref{(alg:ilr_spectral_gsca)}): produces
...
dominant cost is the randomised SVD in Algorithm~\ref{(alg:ilr_spectral_gsca)},
...
Step & Operation & Cost \\
\midrule
\multicolumn{3}{l}{\emph{Algorithm~\ref{(alg:ilr_spectral_gsca)}: Spectral fit}} \\
